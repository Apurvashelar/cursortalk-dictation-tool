...
import { Mic, Send, Waves, CheckCircle2 } from "lucide-react";
...
const VOICE_SCRIPT: TypingOp[] = [
  { type: "pause", ms: 500 },
  { type: "type", text: "Hey team — can we move sprint planning to 3 PM tomorrow?", cps: 18 },
  { type: "pause", ms: 320 },
  { type: "type", text: " Design needs a bit more time on the handoff.", cps: 19 },
  { type: "pause", ms: 260 },
  { type: "type", text: " I’ll send the updated agenda shortly.", cps: 20 },
  { type: "pause", ms: 1100 },
];
...
export default function OpenWhisprSpeechToTextDemo() {
  ...
  const withPlayback = useScriptPlayback(VOICE_SCRIPT);
    ...
    <section className="relative overflow-hidden rounded-[32px] border border-black/10 bg-gradient-to-b from-white to-neutral-50 px-6 py-8 dark:border-white/10 dark:from-neutral-950 dark:to-neutral-900 md:px-8 md:py-10">
      ...
      <div className="relative mb-8 max-w-2xl">
        ...
        <p className="mt-3 max-w-xl text-sm leading-6 text-black/60 dark:text-white/60 md:text-base">
          This version uses a real typing timeline with pauses, typos, backspaces, and corrections to mimic the reference interaction much more closely.
        </p>
      </div>
      ...
      <div className="relative grid gap-5 lg:grid-cols-2">
        ...
        <SpeedCard label="With OpenWhispr" wpm="~150 WPM" tone="accent">
          <ChatComposer value={withPlayback.value} isActive={withPlayback.isActive} withVoice />
        </SpeedCard>
      </div>
    </section>
  );
